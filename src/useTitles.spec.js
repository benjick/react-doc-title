  ...
  return (
    <div>
      <Provider title="react-doc-title">
        ...
        <button id="toggle" type="button" onClick={toggleShow}>
          Toggle
        </button>
        <button id="provider" type="button" onClick={toggleProvider}>
          Toggle
        </button>
        {show && <MyComponent />}
      </Provider>
...
it('can handle nesting of Providers', () => {
  act(() => {
    ReactDOM.render(<TestNestingComponent />, container)
  })
  const toggle = container.querySelector('button#toggle')
  expect(document.title).toBe('test2 - test - react-doc-title')
  act(() => {
